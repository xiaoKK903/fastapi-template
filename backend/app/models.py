...
User.pomodoro_sessions = Relationship(back_populates="owner", cascade_delete=True)
User.pomodoro_settings = Relationship(back_populates="owner", cascade_delete=True)
User.media_collections = Relationship(back_populates="owner", cascade_delete=True)
User.media_tags = Relationship(back_populates="owner", cascade_delete=True)
...
class MediaCollectionBase(SQLModel):
    title: str = Field(min_length=1, max_length=500)
...
class MediaCollectionPublic(MediaCollectionBase):
    id: str
    owner_id: str
    created_at: datetime | None = None
    updated_at: datetime | None = None
...
class MediaCollectionsPublic(SQLModel):
    data: list[MediaCollectionPublic]
    count: int


class MediaCollectionStatistics(SQLModel):
    total_items: int
    want_to_watch: int
    watching: int
    completed: int
...
class MediaTagPublic(MediaTagBase):
    id: str
    owner_id: str
    media_count: int = 0
    created_at: datetime | None = None
...
class MediaTagsPublic(SQLModel):
    data: list[MediaTagPublic]
    count: int


class MediaCollectionTagLink(SQLModel, table=True):
    collection_id: str = Field(
        foreign_key="mediacollection.id", primary_key=True, ondelete="CASCADE"
    )
    tag_id: str = Field(
        foreign_key="mediatag.id", primary_key=True, ondelete="CASCADE"
    )


class MediaTag(MediaTagBase, table=True):
    id: str = Field(default_factory=generate_uuid, primary_key=True)
    owner_id: str = Field(
        foreign_key="user.id", nullable=False, ondelete="CASCADE"
    )
    ...
    collections: list["MediaCollection"] = Relationship(
        back_populates="tags",
        link_model=MediaCollectionTagLink,
    )
    owner: "User" = Relationship(back_populates="media_tags")


class MediaCollection(MediaCollectionBase, table=True):
    id: str = Field(default_factory=generate_uuid, primary_key=True)
    owner_id: str = Field(
        foreign_key="user.id", nullable=False, ondelete="CASCADE"
    )
    ...
    tags: list[MediaTag] = Relationship(
